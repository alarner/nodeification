...
exports.up = function (knex, Promise) {
	return knex.schema.createTable('subscribers', function (t) {
		...
		t.enum('type', ['email', 'text', 'push-web', 'push-android', 'push-ios']);
		t.string('key').notNull();
		t.integer('userId').unsigned().nullable().references('id').inTable('users').onDelete('CASCADE');
		...
		return knex.schema.createTable('subscriptions', function (t) {
			t.increments('id').unsigned().primary();
			t.string('descriptor');
			t.integer('subscriberId').unsigned().nullable().references('id').inTable('subscribers').onDelete('CASCADE');
			t.dateTime('createdAt').notNull();
			t.dateTime('updatedAt').nullable();
		...
		return knex.schema.createTable('unsubscriptions', function (t) {
			t.increments('id').unsigned().primary();
			t.integer('subscriptionId').unsigned().notNull().references('id').inTable('subscriptions').onDelete('CASCADE');
			t.string('reason');
			t.dateTime('createdAt').notNull();
			t.dateTime('updatedAt').nullable();
